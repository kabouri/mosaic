...
class DBRESTConfig(DBConfigBase):

    url: str = pydantic.Field(..., description="REST API URL")


class DBREST(DBBase):

    config: DBRESTConfig = pydantic.Field(None,
                                          description="The data backend configuration")

    def connect(self, status_endpoint="status", logger=None, **params):

        response = requests.get(
            os.path.join(self.config.url, status_endpoint)
        )
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Failed to fetch status, status code: {response.status_code}")
            return None
    ...
    def put(self, endpoint,
        ...
        response = requests.post(
            os.path.join(self.config.url, endpoint),
            data,
        ...
        if response.status_code == 200:
            ...
            if logger:
                logger.info(f"Failed to post data on {endpoint}"
